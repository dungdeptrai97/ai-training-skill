...
**Market Context** (use Simplize MCP Server as primary source for commodity prices, VN-Index data, and stock prices — fall back to web search only if MCP unavailable. **On Mondays: commodity/forex markets trade internationally over the weekend — always fetch CURRENT/REALTIME prices, not last Friday's close. Compare Monday realtime vs Friday close to detect weekend moves.**)
- Overnight futures / pre-market moves
- Sector ETF performance
- Relevant commodity or currency moves
- **⚠️ ALERT if any commodity/currency moves >5% overnight** (oil, steel/HRC, gold, USD/VND, CNY...): flag as TOP CALL, identify VN stocks directly impacted (e.g., oil surge → PVD, PVS, PLX benefit; VJC, HVN hurt), and quantify EPS sensitivity if possible
...
- Major commodity/currency moves (>5%) MUST be the Top Call — these directly impact VN sectors (oil→dầu khí/hàng không, HRC→thép, USD/VND→xuất khẩu) and should never be missed
