...
- For each trend/driver, name specific companies best positioned to benefit and explain why
...
**Value Chain & Margin Leaders**
- Conclude which companies have the best gross margins and operating cash flow, and identify those with the longest/most integrated value chain (strongest sustainable competitive advantages)
...
- Use MCP server as primary source for company financial data and broker/securities company research reports — only fall back to web search if MCP data is unavailable
- Search for latest sector analysis reports from securities companies (via MCP) to cross-reference market size, competitive landscape, and growth forecasts
